...
class TravellerForm(forms.Form):
    full_name = forms.CharField(
        ...
        widget=forms.TextInput(attrs={'class': 'form-control', 'id': 'full_name', 'placeholder': "Write full name..."}),required=True)
    ...
    nationality = forms.ModelChoiceField(queryset=Location.objects.filter(parent=0) , empty_label='-- Select--',
                            widget=forms.Select(attrs={'class': 'form-control', 'id': 'nationality'}), required=True) #todo: review it
    ...
    point_of_entry = forms.ModelChoiceField(queryset=PointOfEntry.objects.all(), empty_label='-- Select--',
                                            widget=forms.Select(attrs={'class': 'form-control', 'id': 'point_of_entry'}), required=True)

    mode_of_transport = forms.ChoiceField(label='Mode of Transport', choices=TRANSPORT_MODE, widget=forms.Select(
        attrs={'class': 'form-control', 'id': 'mode_of_transport'}), required=True)
    ...
    visiting_purpose = forms.ChoiceField(label='Visiting Purpose', choices=PURPOSE,
                                         widget=forms.Select(attrs={'class': 'form-control', 'id': 'visiting_purpose'}), required=True)
    ...
    employment = forms.ChoiceField(label='Employment/Occupation', choices=EMPLOYMENT,
                                   widget=forms.Select(attrs={'class': 'form-control', 'id': 'employment'}), required=False)
    ...
    region_id = forms.ModelChoiceField(label='Region',
                                             queryset=Location.objects.filter(parent=2), empty_label='-- Select --',
                                             widget=forms.Select(
                                                 attrs={'class': 'form-control', 'id': 'region_id', 'onChange': 'suggest_districts()'}))

    district_id = forms.ChoiceField(label='District', choices=blank_choice,
                                    widget=forms.Select(attrs={'class': 'form-control', 'id': 'district_id'}), required=False)  # todo: review it

    street_or_ward = forms.CharField(label='Street/Ward', widget=forms.TextInput(
        attrs={'class': 'form-control', 'id': 'street_or_ward', 'placeholder': 'Write street or ward...'}), required=False)

    phone = forms.CharField(label='Phone', widget=forms.TextInput(
        attrs={'class': 'form-control', 'id': 'phone', 'placeholder': 'Write phone...'}))
    ...
    location = forms.ModelChoiceField(label='Country', queryset=Location.objects.filter(parent=0), empty_label='-- Select --',
                                      widget=forms.Select(
    ...
    date = forms.CharField(label='Date', widget=forms.DateInput(
        attrs={'class': 'form-control', 'id': 'datepicker', 'placeholder': 'Pick date...'}))
    ...
    # tab 5
    symptom = forms.MultipleChoiceField(widget=forms.CheckboxSelectMultiple(
       attrs={'id': 'symptom'}), choices=Symptom.objects.all().values_list("id", "title"))
