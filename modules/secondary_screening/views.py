...
from .models import RiskAssessment, DiseaseSurveyQns, DiseaseSurveyAns
from modules.travellers.models import Traveller, Disease, TravellerVisitedArea
...
def traveller_info(request, travellers_id):
    traveller = Traveller.objects.select_related('nationality', 'region', 'location_origin',
                                                 'point_of_entry').get(pk=travellers_id)
    disease_to_screen = traveller.disease_to_screen.split(",")
    diseases = Disease.objects.filter(pk__in=disease_to_screen).values()
    ...
    visited_countries = TravellerVisitedArea.objects.select_related('location').filter(
        traveller_id=travellers_id).values("location__title", "location_visited", "days", "date")
...
def screen_list(request):
    ...
    travellers = (Traveller.objects
                  ...
                  .values('id', 'other_names', 'surname', 'nationality__title', 'id_number',
                          'age_category', 'age', 'sex', 'temp', 'name_of_transport', 'disease_to_screen',
...
def survey(request, travellers_id, disease_id):
    ...
    answers = DiseaseSurveyAns.objects.filter(disease_id=disease_id, traveller_id=travellers_id) \
